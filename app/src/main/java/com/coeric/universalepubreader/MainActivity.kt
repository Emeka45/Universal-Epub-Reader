...
class MainActivity : Activity() {
        ...
        webView = WebView(this).apply {
            ...
            webViewClient = object : WebViewClient() {
                override fun shouldOverrideUrlLoading(view: WebView, request: WebResourceRequest): Boolean {
                    return handleChapterLink(request.url)
                }

                @Suppress("DEPRECATION")
                override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
                    return handleChapterLink(Uri.parse(url))
                }
            }
        }
        root.addView(bar)
    ...
    private fun loadEpub(uri: Uri) {
        ...
        Thread {
            try {
                ...
                var parsedChapters = emptyList<String>()
                var parsedTitles = emptyList<String>()
    ...
    private fun showChapter(index: Int) {
        ...
        webView.loadDataWithBaseURL(
            file.parentFile?.toURI()?.toString(),
            buildChapterHtml(file, index),
            "text/html",
            "UTF-8",
            null
        )
    ...
    private fun buildChapterHtml(file: File, index: Int): String {
        ...
        doc.select("a[href]").forEach { anchor ->
            ...
            val targetIndex = target?.let { targetFile -> chapters.indexOfFirst { sameFile(it, targetFile) } }
            if (targetIndex >= 0) {
                anchor.attr("href", "chapter://$targetIndex")
    ...
    private fun find(query: String) {
        ...
        Thread {
            val found = chapters.indexOfFirst { path ->
                runCatching {
                    Jsoup.parse(File(path), "UTF-8").text().contains(normalized, ignoreCase = true)
                }.getOrDefault(false)
            }
            runOnUiThread {
                if (found >= 0) {
    ...
    private fun chapterTitle(file: File, index: Int): String {
        return runCatching {
            val doc = Jsoup.parse(file, "UTF-8")
            doc.title().trim().ifBlank {
    ...
    private fun sameFile(path: String, target: File): Boolean =
        runCatching { File(path).canonicalFile == target.canonicalFile }.getOrDefault(false)
    ...
    private fun <K, V> Iterable<K>.associateNotNull(transform: (K) -> Pair<K, V>?): Map<K, V> =
